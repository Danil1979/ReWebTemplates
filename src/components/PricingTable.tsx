...
import { Maximize2, Tag, ArrowRight, Maximize } from 'lucide-react';
    ...
    <section id="pricing" className="py-32 bg-transparent relative overflow-hidden">
      <div className="page-wrap px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-7 reveal-up">
            <div 
              className={`bg-white/80 backdrop-blur-2xl rounded-[3rem] p-10 md:p-16 border border-white shadow-3xl relative group ${pricing[activePlan].layoutImage ? 'cursor-pointer' : ''}`}
              onClick={() => {
                if (pricing[activePlan].layoutImage) {
                  setIsModalOpen(true);
                }
              }}
            >
                ...
                {pricing[activePlan].layoutImage ? (
                  <div className="relative w-full h-full">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                      <div className="w-16 h-16 rounded-full bg-[var(--forest-deep)]/5 backdrop-blur-sm border border-[var(--forest-deep)]/10 flex items-center justify-center text-[var(--forest-deep)]">
                        <Maximize size={24} />
                      </div>
                    </div>
                  </div>
      ...
      {pricing[activePlan].layoutImage && (
        <ImageModal 
          ...
          imageUrl={pricing[activePlan].layoutImage!}
          caption={`${pricing[activePlan].unitType} Floor Plan`}
        />
      )}
    </section>
  );
